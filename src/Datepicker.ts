...
class Datepicker {
	...
	private createBody = (): HTMLElement => {
		...
		for (let i = 0; i < offsetStartWeek; i++) {
			days.innerHTML += `<span class="${options.classNames.dayItemClassName} ${
				options.classNames.dayItemClassName
			}--disabled"></span>`;
		}
		...
		for (let i = 1; i <= daysInCurrentMonth.length; i++) {
			...
			days.innerHTML += `<span data-date="${dateValue}" class="${
				options.classNames.dayItemClassName
			} ${todayClass} ${selectedDate}">${i}</span>`;
		}

		weeks.classList.add(options.classNames.weeksClassName);
		for (let i = 0; i < weekName.length; i++) {
			weeks.innerHTML += `<span class="${options.classNames.weekItemClassName}">${
				weekName[i]
			}</span>`;
		}
